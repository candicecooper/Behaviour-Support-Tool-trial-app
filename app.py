...
MOCK_STAFF = [
    ...
    # *** FIX APPLIED HERE: Corrected dictionary syntax ***
...
def set_landing_page_background(image_file):
    ...
    Includes NEW CSS for the clickable role boxes.
    """
    try:
        ...
        /* Remove duplicate Streamlit title/header element */
        header {{
            display: none !important;
        }}

        /* 3. Custom button styling for Quick Log/Direct Log buttons */
        ...
        /* 5. Placeholder for the three images/columns (Feature Cards) */
        .image-placeholder {{
        ...
        /* === CSS FOR CLICKABLE ROLE BOXES (Identical to Feature Cards) === */
        
        /* 6. Wrapper for Role Boxes to maintain consistent height and position the overlay */
        .role-card-wrapper {{
            position: relative; 
            height: 250px; 
            margin-bottom: 20px;
        ...
        /* Content box inherits the look of the feature cards */
        .role-card-content {{
            background-color: rgba(44, 62, 80, 0.7); 
            padding: 20px;
            border-radius: 10px;
        ...
        /* Hover effect applied to the wrapper to target the content */
        .role-card-wrapper:hover .role-card-content {{
            background-color: rgba(52, 73, 94, 0.8); 
            border: 2px solid #00FFFF; 
            transform: translateY(-3px);
            box-shadow: 0 8px 16px rgba(0, 0, 0, 0.6);
        ...
        /* The overlay div for the button */
        .role-card-button-overlay {{
            position: absolute;
            top: 0;
            left: 0;
            height: 100%;
            width: 100%;
            z-index: 10; /* Ensures the button captures clicks */
        }}
        
        /* CRITICAL FIX 1: Target the button's wrappers to force 100% size and zero padding/margin */
        .role-card-button-overlay > div:first-child,
        .role-card-button-overlay > div:first-child > div {{
            height: 100% !important;
            width: 100% !important;
            margin: 0 !important;
            padding: 0 !important;
            /* Ensure the Streamlit padding around the button is also gone */
        }}
        
        /* CRITICAL FIX 2: Target the actual button element */
        .role-card-button-overlay button {{
            height: 100% !important;
            width: 100% !important;
            margin: 0 !important;
            padding: 0 !important;
            ...
            transition: none !important; /* Remove any transition which might cause flicker */
        }}
        
        /* CRITICAL FIX 3: Ensure the button's hover state is also completely invisible */
        .role-card-button-overlay button:hover {{
            background-color: transparent !important;
            border-color: transparent !important;
            opacity: 0 !important;
        }}
        
        /* CRITICAL FIX 4: Ensure the button label (the space " ") is absolutely invisible */
        .role-card-button-overlay button span {{
            display: none !important;
        }}
...
def render_role_select():
    """Renders the four main area buttons as identical, clickable boxes."""
    st.markdown("### Select Your Primary Area to Login:")
    
    col_jp, col_py, col_sy, col_adm = st.columns(4)
    
    # Data structure for the new role boxes
    role_data = [
        {"label": "Junior Primary", "role": "JP", "desc": "Junior Primary data and management.", "key": "JP_login"},
        {"label": "Primary Years", "role": "PY", "desc": "Primary Years data and management.", "key": "PY_login"},
        {"label": "Senior Years", "role": "SY", "desc": "Senior Years data and management.", "key": "SY_login"},
        {"label": "Admin Portal", "role": "ADM", "desc": "Administrator-level data access and controls.", "key": "ADM_login"},
    ]
    
    cols = [col_jp, col_py, col_sy, col_adm]
    
    for i, data in enumerate(role_data):
        with cols[i]:
            # 1. Use the new HTML structure to mimic the box visually
            st.markdown(
                f"""
                <div class="role-card-wrapper">
                    <div class="role-card-content">
                        <h4 style="margin: 0; padding-top: 10px;">{data['label']}</h4>
                        <p style="font-size: 0.85em; text-align: center; margin-top: 5px;">{data['desc']}</p>
                        <h3 style="color: #00FFFF;">{data['role']}</h3>
                        <p style="font-size: 0.8em;">**Login Now**</p>
                    </div>
                    <div class="role-card-button-overlay">
                        {st.button(
                            label=" ", # Empty label, as the button is invisible
                            key=data['key'], 
                            use_container_width=True, 
                            on_click=navigate_to, 
                            args=('staff_area',), 
                            kwargs={'role': data['role'], 'mode': 'home'}
                        )}
                    </div>
                </div>
                """, 
...
def render_direct_log_select():
    ...
    if selected_student_name:
        ...
        # Label changed from 'Quick Log' to 'Start Log' for this specific log
...
def render_landing_page():
    ...
    with st.container(border=False):
        ...
        with col_content:
            # --- 2. Feature Cards (Non-Clickable Visuals) ---
            ...
            # --- 3. Role/Area Selection (Now using identical clickable boxes) ---
            st.markdown("---")
            render_role_select()
